...
- Concentrations inside `pdCalculator` are **mg/L of plasma water** (urea distributes only into body water, and Watson's V is total body water — so the mass-balance equations naturally track plasma-water concentration). The UI displays **mg/dL of plasma water** — just `/ 10` at the boundary (`updateAllResults`, `updateGraphWithTreatment`). No 0.93 factor is applied at the display boundary; the displayed slope satisfies `slope × V_water = gen` directly, which is what the Excel upslope test (#2) checks. Note that lab-reported BUN is in *whole plasma* convention (~7% lower than plasma water); if you switch the display to whole plasma, multiply by 0.93 at the boundary and update the back-calc test formula accordingly.
- `kru` and `mtac` are **mL/min**. The `kru` input is treated as **whole plasma** clearance and converted to **plasma water** inside `pdCalculator` via `kru = kru * 0.93` ([controller.js:143](controller.js:143)) — plasma-water clearance is *smaller* than whole-plasma because urea is concentrated in the water fraction (~93% of plasma), so `K_pw = 0.93 × K_wp`. This is a local parameter reassignment, so the "Kurea" cell in the results table still displays the original whole-plasma input.
